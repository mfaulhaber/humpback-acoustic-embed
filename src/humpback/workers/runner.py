...
async def run_worker(settings: Settings | None = None) -> None:
    ...
    while not shutdown.is_set():
        ...
        async with session_factory() as session:
            ...
            cftjob = await claim_classifier_feedback_training_job(session)
        if cftjob:
            logger.info(f"Classifier feedback training job {cftjob.id}")
            from humpback.workers.event_classifier_feedback_worker import (
                run_event_classifier_feedback_training,
            )

            async with session_factory() as session:
                await run_event_classifier_feedback_training(session, cftjob, settings)
            claimed = True
