...
class EmbeddingModel(Protocol):
    ...
    def embed(self, windows: np.ndarray) -> np.ndarray:
        """Embed a batch of windows. Input: (batch, samples). Output: (batch, vector_dim)."""
        ...


class FakeTFLiteModel:
    """Deterministic fake model for testing. Returns sin/cos embeddings based on window index."""
    ...
    def embed(self, windows: np.ndarray) -> np.ndarray:
        ...
        for i in range(batch_size):
            # Deterministic embedding based on window content hash
            seed = int(np.abs(windows[i, :8]).sum() * 10000) % (2**31)
...
class TFLiteModel:
    ...
    def embed(self, windows: np.ndarray) -> np.ndarray:
        ...
        for i in range(windows.shape[0]):
            inp = windows[i : i + 1].astype(np.float32)
