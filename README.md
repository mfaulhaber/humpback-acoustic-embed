...
- Sequence Models track (parallel to Call Parsing): two parallel embedding sources — SurfPerch event-padded chunks and Pass-1 region-scoped CRNN BiGRU chunk embeddings — feeding a shared PCA + Gaussian HMM consumer with three training modes (`full_region`, `event_balanced`, `event_only`) for the CRNN source. State timeline viewer with PCEN spectrogram, transition matrix heatmap, dwell-time histograms, per-state tier-composition strip for CRNN source, motif extraction over collapsed Viterbi-state n-grams, and PCA/UMAP overlay + label distribution + exemplar gallery interpretation views (SurfPerch source for Phase 1)
- Masked-transformer sequence model (parallel to HMM Sequence): masked-span PyTorch transformer trained on CRNN region-based continuous embeddings, producing contextual embeddings tokenized via per-k k-means with softmax-temperature confidence. One training job supports a configurable k-sweep (`extend-k-sweep` adds new k values without retraining). MPS/CUDA training with synthetic-batch validation + CPU fallback. Detail page provides a URL-synced k-picker, loss curve, timeline strips (token + token-confidence + reconstruction-error), token run-length histograms, and the same interpretation visualizations + motif-extraction panel as HMM
