...
After prominence peak selection, a recursive gap-filling pass scans for gaps > 3.0 seconds between consecutive selected peaks (and from event edges to the nearest peak). For each gap, the highest raw-probability candidate above `min_score` is emitted, splitting the gap into two sub-gaps that are checked recursively. This is always-on in prominence mode; the 3.0-second threshold is hardcoded (not an API parameter).
