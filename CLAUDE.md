...
- Window classification sidecar: standalone enrichment scoring cached Pass 1 Perch embeddings through vocalization classifiers, producing dense per-window probability vectors with review workspace for inspection and correction
- Classify review boundary editing: adjust/add/delete event boundaries in Pass 3 review, corrections flow to both classification inference and classifier feedback training via read-time overlay (ADR-054)
...
- **Latest migration**: `053_window_classification_tables.py`
