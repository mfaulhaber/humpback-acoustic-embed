...
- Sequence Models track (parallel to Call Parsing): continuous embedding producer that emits region-bounded, hydrophone-only 1-second-hop SurfPerch embeddings padded around Pass-1 detections, idempotent on `encoding_signature` (ADR-056). HMM training (PCA + GaussianHMM via hmmlearn) with Viterbi decode, state timeline visualization, transition matrix heatmap, and dwell-time histograms. Motif mining lands in subsequent PRs.
...
- **Latest migration**: `058_hmm_sequence_jobs.py`
