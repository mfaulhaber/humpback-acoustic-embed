...
- `gain_norm_threshold_db` defaults to `6.0`. `HUMPBACK_GAIN_NORM_THRESHOLD_DB` controls the RMS jump (dB above median) required to flag a region as a recording gain change.
- `gain_norm_min_duration_sec` defaults to `5.0`. `HUMPBACK_GAIN_NORM_MIN_DURATION_SEC` controls the minimum sustained duration to distinguish gain changes from whale calls or transient noise.
...
- **Timeline gain normalization**: timeline tile rendering and audio playback apply automatic gain normalization to correct abrupt mic gain changes in hydrophone recordings. A per-job gain profile (`.gain_profile.json` in timeline cache) is computed once by walking 1-second RMS windows and detecting sustained high-gain regions. Attenuation is applied to raw audio before spectrogram generation and audio encoding. The gain profile is computed before `ref_db` so that `ref_db` reflects corrected audio levels. Source audio is never modified. Clearing a job's timeline cache folder resets the gain profile.
...
- Hyperparameter tuning: UI-driven manifest generation from training/detection jobs, random hyperparameter search with configurable search space, production model comparison, candidate import from search results. Scripts in `scripts/autoresearch/` are thin CLI wrappers over `humpback.services.hyperparameter_service`
