...
- `pcen_time_constant_sec` (`0.5`), `pcen_gain` (`0.98`), `pcen_bias` (`10.0`), `pcen_power` (`0.25`), `pcen_eps` (`1e-6`): PCEN parameters applied per timeline tile via `librosa.pcen`. Overridable via `HUMPBACK_PCEN_*`.
- `pcen_warmup_sec` defaults to `2.0`. Each tile's audio fetch is extended backward by this amount so the PCEN low-pass filter can settle before the first rendered frame; the warm-up frames are trimmed off the output.
- `pcen_vmin` (`0.0`) / `pcen_vmax` (`0.15`): fixed colormap range for PCEN-normalized tiles. Because PCEN's output is bounded, there is no per-job `ref_db` computation.
