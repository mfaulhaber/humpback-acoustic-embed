...
- Sequence Models track (parallel to Call Parsing): two parallel embedding sources (ADR-056, ADR-057). (1) SurfPerch event-padded source — continuous embedding producer that emits event-scoped, hydrophone-only 1-second-hop SurfPerch embeddings padded around Pass-2 segmentation events (each event is an independent span — no merging), idempotent on `encoding_signature`. (2) CRNN region-based source — emits 250 ms chunk embeddings per Pass 1 detection region from the Pass 2 segmentation CRNN's BiGRU activations, with per-chunk metadata (`call_probability`, `event_overlap_fraction`, `nearest_event_id`, `tier`) joined against the matching Pass 2 events; three training modes (`full_region`, `event_balanced`, `event_only`) selectable on the HMM job. HMM training (PCA + GaussianHMM via hmmlearn) with Viterbi decode, state timeline visualization with dual region/event navigation (A/D keyboard shortcuts), transition matrix heatmap, and dwell-time histograms. Interpretation visualizations: PCA/UMAP overlay scatter colored by HMM state and per-state exemplar gallery (high-confidence, nearest-centroid, boundary picks) — both unified across SurfPerch and CRNN sources via the source-agnostic loader Protocol (ADR-059); CRNN exemplar cards carry an additional `extras.tier` badge. State-to-label distribution is sourced from the bound `EventClassificationJob` (Pass 3 Classify) plus `VocalizationCorrection` overlay (ADR-063, supersedes ADR-060): event-scoped inversion tags each sequence window by center-time membership in effective events, the chart shape is flat per-state `{label: count}` with a reserved `(background)` bucket for windows outside any event, and CRNN's `extras.tier` persists on `decoded.parquet`/exemplars but no longer stratifies the chart. HMM/MT submit endpoints accept an optional `event_classification_job_id` (defaults to most recent completed for the upstream segmentation; rejected if none); a manual "Regenerate label distribution" button on the detail pages rebuilds artifacts and optionally re-binds atomically (validate → temp-then-rename → commit FK). Exemplar cards render type chips that click through to Classify Review for the underlying event. CRNN-source HMM jobs render a per-state tier-composition stacked-bar strip. Motif extraction jobs (ADR-058) mine collapsed Viterbi-state n-grams, rank recurring motifs, and show event-midpoint aligned examples on the HMM detail page.
- Masked-transformer sequence model (ADR-061), parallel to HMM Sequence and consuming the CRNN region-based continuous embeddings. One training job per (upstream, training config); per-k k-means tokenization fan-out under `k<N>/` subdirs (decoded.parquet, kmeans.joblib, overlay.parquet, exemplars.json, label_distribution.json, run_lengths.json) with atomic `k<N>.tmp/` → rename writes. Training-signature idempotency excludes `k_values`, so a completed job exposes `extend-k-sweep` to add new k values without retraining the transformer. Mask-weight bias upweights event-adjacent positions in the loss; k-means confidence uses softmax with τ auto-fit per job and drops into the existing `max_state_probability` UI slot. Training runs on MPS/CUDA with synthetic-batch validation against CPU and CPU fallback on validation failure. Loader stack normalized on `decoded.parquet` (column `label`, renamed from HMM's `state`); `motif_extraction_jobs` generalized via `parent_kind` discriminator + XOR parent FKs + parent-kind-conditional `k`, so motifs work uniformly across HMM and masked-transformer parents. Frontend extracts a generic `DiscreteSequenceBar` (modes `rows` / `single-row`) and `RegionNavBar` shared with the HMM detail page; new "Masked Transformer" Sequence Models nav card with URL-synced `?k=` picker on the detail page. Token Count selector (`2 | 3 | 4`) on the timeline legend surveys all length-N motifs at once: every visible occurrence is highlighted with a deterministic per-motif color and prev/next/Play in the legend walks the visible set; mutually exclusive with the single-motif row selection in the Motifs panel.
- Web UI: Classifier (Training, Hydrophone Detection, Embeddings, Tuning, Labeling), Vocalization (Training, Labeling, Training Data, Clustering), Call Parsing (Detection, Segment, Segment Training, Classify, Classify Training, Window Classify), Sequence Models (Continuous Embedding, HMM Sequence, Masked Transformer), Admin

### 9.2 Database Schema

- **Engine**: SQLite via SQLAlchemy
- **Latest migration**: `066_sequence_models_classify_binding.py` (adds nullable `event_classification_job_id` FK + index on `hmm_sequence_jobs` and `masked_transformer_jobs`; binds Sequence Models label source to Call Parsing Pass 3 Classify, ADR-063)
- **Tables**: model_configs, audio_files, clustering_jobs, clusters, cluster_assignments, classifier_models, classifier_training_jobs, autoresearch_candidates, detection_jobs, retrain_workflows, vocalization_labels, vocalization_types, vocalization_models, vocalization_training_jobs, vocalization_inference_jobs, detection_embedding_jobs, training_datasets, training_dataset_labels, hyperparameter_manifests, hyperparameter_search_jobs, call_parsing_runs, segmentation_models, region_detection_jobs, event_segmentation_jobs, event_classification_jobs, window_classification_jobs, vocalization_corrections, event_boundary_corrections, segmentation_training_datasets, segmentation_training_samples, segmentation_training_jobs, region_boundary_corrections, event_classifier_training_jobs, continuous_embedding_jobs, hmm_sequence_jobs, motif_extraction_jobs, masked_transformer_jobs
