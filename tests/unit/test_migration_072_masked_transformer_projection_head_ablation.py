"""Tests for migration 072 (masked-transformer projection-head ablation)."""
...
def _create_pre_072_schema(db_path: Path) -> None:
    conn = sqlite3.connect(db_path)
    try:
        conn.executescript(
            """
            ...
            INSERT INTO alembic_version (version_num) VALUES ('071');
            """
        )
        conn.commit()
    finally:
...
def test_upgrade_adds_projection_head_ablation_columns(tmp_path: Path) -> None:
    db_path = tmp_path / "test.db"
    _create_pre_072_schema(db_path)

    command.upgrade(_alembic_config(db_path), "072")

    cols = _columns(db_path)
    assert cols["training_freeze_mode"]["notnull"] is True
    assert cols["training_freeze_mode"]["default"] == "'none'"
    assert cols["source_masked_transformer_job_id"]["notnull"] is False
    assert cols["negative_label_family_policy_json"]["notnull"] is False

    conn = sqlite3.connect(db_path)
    try:
        row = conn.execute(
            "SELECT training_freeze_mode, source_masked_transformer_job_id, "
            "negative_label_family_policy_json FROM masked_transformer_jobs "
            "WHERE id='mt-1'"
        ).fetchone()
    finally:
        conn.close()
...
def test_downgrade_removes_projection_head_ablation_columns(tmp_path: Path) -> None:
    db_path = tmp_path / "test.db"
    _create_pre_072_schema(db_path)
    ...
    cols = _columns(db_path)
    assert "training_freeze_mode" not in cols
    assert "source_masked_transformer_job_id" not in cols
    assert "negative_label_family_policy_json" not in cols
