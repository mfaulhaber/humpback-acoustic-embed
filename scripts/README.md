...
# masked_transformer_retrieval_sweep

Plan, submit, and compare retrieval-aware masked-transformer research sweeps.
The script is a coordinator only: submit mode calls the existing
`create_masked_transformer_job()` service and never starts worker loops;
compare mode calls backend nearest-neighbor diagnostics and writes research
artifacts.

Basic dry-run for the first Phase 5 sweep preset:

```bash
uv run python scripts/masked_transformer_retrieval_sweep.py submit \
    --dry-run \
    --preset initial-retrieval-aware-sweep \
    --continuous-embedding-job-id-250ms CEJ_250MS \
    --continuous-embedding-job-id-100ms CEJ_100MS \
    --event-classification-job-id CLASSIFY_JOB \
...
uv run python scripts/masked_transformer_retrieval_sweep.py submit \
    --preset lambda \
    --continuous-embedding-job-id CEJ_250MS \
    --event-classification-job-id CLASSIFY_JOB \
    --lambda-values 0.05,0.10,0.25,0.50 \
    --k-values 150 \
    --output-dir data/retrieval_sweeps
```

Compare a manifest or explicit jobs:

```bash
uv run python scripts/masked_transformer_retrieval_sweep.py compare \
    --manifest data/retrieval_sweeps/submit-manifest.json \
    --include-known-metrics \
    --output-dir data/retrieval_sweeps

uv run python scripts/masked_transformer_retrieval_sweep.py compare \
    --job baseline=9fd95e63-9f06-4cfb-8242-63a03dbbedd0:contextual \
    --job trial=MASKED_TRANSFORMER_JOB_ID:retrieval \
    --k 150
```

Outputs are deterministic by default:

* `submit-manifest.json`: planned rows, submitted/reused job ids, blocked rows,
  and `label_semantics="authoritative_single_human_label"`.
* `comparison.csv`, `comparison.md`, `comparison.json`: ranked
  `exclude_same_event_and_region` retrieval diagnostics with raw, remove-PC,
  whitened, event-level, verdict, and label-cardinality columns.

True hard-negative policies are intentionally not accepted by this script until
the masked-transformer job schema grows persisted hard-negative fields.
