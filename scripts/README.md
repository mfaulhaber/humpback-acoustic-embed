# masked_transformer_nn_report

Build a nearest-neighbor diagnostic report for a completed masked-transformer
Sequence Models job. The script reads `HUMPBACK_DATABASE_URL` and
`HUMPBACK_STORAGE_ROOT` from `.env`, loads the job's contextual embeddings,
joins chunk metadata from the upstream continuous-embedding artifact, and uses
human vocalization corrections as the primary call-type labels.

Basic usage:

```bash
uv run python scripts/masked_transformer_nn_report.py \
    --job-id 9fd95e63-9f06-4cfb-8242-63a03dbbedd0 \
    --k 150 \
    --samples 50 \
    --topn 10
```
...
Outputs are written under `.tmp/nn-report/` by default:

* Markdown report with aggregate tables and representative query examples.
* Query-level CSV with per-query rates and verdicts.
* Neighbor-level CSV with all top-k neighbors, cosine scores, labels, event
  duration, token, tier, event-overlap, and adjacency flags.
* Metadata JSON with job inputs, correction coverage, artifact paths, and
  optional sweep metrics.

Important options:

* `--pca-diagnostics` adds PC1, PC1-PC5, PC1-PC10, and PC1-PC50 explained
  variance metrics for raw contextual embeddings after mean-centering.
* `--mean-centered-comparison` compares raw L2-normalized contextual embeddings
  against mean-centered then L2-normalized embeddings.
* `--pc-removal-sweep` compares `raw_l2`, `centered_l2`, and residual spaces
  after removing the top `1,3,5,10` principal components. Use
  `--pc-removal-counts` to override the component counts.
* `--whitening-sweep` adds a PCA-whitened variant, centered first and then
  L2-normalized. Use `--whiten-components` to change the default `128`
  components.
* `--exclude-same-event-region` adds a separate retrieval treatment that masks
  candidate neighbors from the same event or same region as each query.

The primary quality signal is `same human label overlap`, computed from
overlapping `vocalization_corrections` `add` rows minus overlapping `remove`
rows, assigned to boundary-corrected effective events. Classifier-overlay
labels are retained as secondary context because broad multi-label classifier
output can be less diagnostic for nearest-neighbor quality.
