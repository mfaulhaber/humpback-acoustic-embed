...
**Masked Transformer detail page panel order** (top → bottom, after the metadata Card and KPicker): Token Timeline (plain Card with the legend inside), Motifs, Loss Curve, Run-Length Histograms, Overlay, Exemplars, Label Distribution. Motifs sits directly under the timeline so the legend, prev/next, and the alignment-strip are all visible without scrolling. In the Exemplars panel, non-background vocal label badges center the Token Timeline in-place on the exemplar midpoint and scroll the timeline card into view; they are not the primary Classify Review navigation affordance.
