...
- `EventEncoderJobsPage`, `EventEncoderCreateForm`, `EventEncoderJobTable`, and `EventEncoderDetailPage` are the active Event Encoder UI surface. `EventEncoderDetailPage` includes an `EventEncoderTimelinePanel` as its second panel for read-only tokenized-event review with region-backed spectrogram tiles, audio playback, zoom controls, k selection, event navigation, and token badges.
