...
**Navigation**: Side nav + top nav layout with react-router-dom. Classifier has sub-routes (`/app/classifier/training`, `/app/classifier/hydrophone`, `/app/classifier/embeddings`, `/app/classifier/tuning`, `/app/classifier/labeling`); Vocalization has sub-routes (`/app/vocalization/training`, `/app/vocalization/labeling`, `/app/vocalization/training-data`); the timeline viewer is at `/app/classifier/timeline/:jobId`; other sections are single-route pages. The Tuning page (`TuningTab.tsx`) has three collapsible sections: Manifests (create/list/detail/delete data manifests), Searches (create/list/detail/delete hyperparameter search jobs with configurable search space), and Candidates (relocated `AutoresearchCandidatesSection` for artifact import, review, and candidate-backed promotion).
