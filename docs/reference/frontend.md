...
**Navigation**: Side nav + top nav layout with react-router-dom. Classifier has sub-routes (`/app/classifier/training`, `/app/classifier/hydrophone`, `/app/classifier/embeddings`, `/app/classifier/labeling`); Vocalization has sub-routes (`/app/vocalization/training`, `/app/vocalization/labeling`, `/app/vocalization/training-data`); the timeline viewer is at `/app/classifier/timeline/:jobId`; other sections are single-route pages. The classifier Training page now combines manual embedding-set training, model/retrain tables, and an `AutoresearchCandidatesSection` for artifact import, review, and candidate-backed promotion.
