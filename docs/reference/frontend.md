...
**Masked Transformer create form**: Advanced options include a retrieval projection-head checkbox. When enabled, `retrieval_dim` and `retrieval_hidden_dim` numeric controls are active and default to 128 / 512, with an L2-normalize checkbox defaulting on. The unchecked default submits `retrieval_head_enabled=false` and null dimensions so existing contextual masked-transformer jobs keep their prior behavior. The same panel includes a sequence-construction radio group (`region`, `event_centered`, `mixed`). Region is the default and submits `event_centered_fraction=0.0` with null context fields. Event-centered and mixed modes enable non-negative pre/post context controls defaulting to 2.0 seconds; mixed mode additionally enables `event_centered_fraction` and requires `0.0 < fraction < 1.0`. Human-correction contrastive controls remain disabled until the retrieval head is enabled; when checked, the form submits `contrastive_label_source="human_corrections"` plus loss weight, temperature, support thresholds, and the cross-region-positive flag. The default submit remains contrastive-disabled with `contrastive_loss_weight=0.0` and `contrastive_label_source="none"`.
