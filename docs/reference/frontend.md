...
- `frontend/src/api/sequenceModels.ts` exports Continuous Embedding, Event Encoder, and Piano Roll Notes types, fetchers, and TanStack Query hooks. Piano Roll Notes hooks (`usePianoRollNotesStatus`, `usePianoRollNotes`, `useCreatePianoRollNotesJob`) keep refetching `notes-status` while queued or running, and invalidate the status and timeline queries on mutation success.
