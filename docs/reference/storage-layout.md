...
/event_encoders/
  ...
  {job_id}/event_notes_{extractor_version}.parquet  (Piano Roll Notes sidecar; current default is v4 — HPS-based F0 extractor from ADR-070. v3 and v4 rows share an identical schema (`note_uid`, `f0_track_id`, `contour_frame_count` plus the v2 columns); legacy v1/v2/v3 sidecars remain readable on disk until manually deleted)
  {job_id}/event_ridges_{tokenizer_version}.parquet (Per-event STFT ridge contour: one row per frame per event with `event_id`, `frame_index`, `frame_time_offset_s`, `log_frequency`, `strength`, `energy_ratio`. Produced by the encoder worker; consumed by the Piano Roll Notes v3 and v4 extractors — ADR-069 §6.1)
...
/exports/
  event_encoders/{job_id}/notes_{extractor_version}.mid    (Piano Roll Notes MIDI for the last-exported window — produced atomically with audio_{extractor_version}.flac. v3 and v4 use MPE Lower Zone with per-voice pitch bend; legacy v1/v2 use the slim seven-channel layout from ADR-067. ADR-068, ADR-069, ADR-070)
