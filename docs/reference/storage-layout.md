...
/masked_transformer_jobs/
  ...
  {job_id}/retrieval_head_outputs.parquet (optional; retrieval-head jobs only; raw projection-head vectors before optional L2 normalization, used by geometry diagnostics)
  {job_id}/loss_curve.json             (per-epoch train + val total loss history; Phase 3 also stores masked/contrastive/total series and contrastive skipped-batch counts)
