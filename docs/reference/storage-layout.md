...
/event_encoders/
  ...
  {job_id}/event_notes_{extractor_version}.parquet  (Piano Roll Notes sidecar; current default is v7 — v6's de-spiked harmonic-Viterbi decode plus residual discontinuity splitting and ridge-guided flat-segment rescue from ADR-074. v3, v4, v5, v6, and v7 rows share an identical schema (`note_uid`, `f0_track_id`, `contour_frame_count` plus the v2 columns); legacy v1-v6 sidecars remain readable on disk until manually deleted)
  {job_id}/event_ridges_{tokenizer_version}.parquet (Per-event STFT ridge contour: one row per frame per event with `event_id`, `frame_index`, `frame_time_offset_s`, `log_frequency`, `strength`, `energy_ratio`. Produced by the encoder worker; consumed by the Piano Roll Notes v3 and v4 extractors and by v7 ridge rescue. v5 and v6 accept the sidecar for signature parity but do not consume it — F0 is derived directly from the CQT (ADR-071, ADR-072, ADR-074) — ADR-069 §6.1)
  ...
  {job_id}/event_note_contours_v6.parquet           (Per-frame note contour sidecar for v6 notes. Schema is identical to the v3 / v4 / v5 sidecar; the F0 contour is v5's de-spiked, and `subharmonic_octave` is reserved / unused and always written as 0 — ADR-072)
  {job_id}/event_note_contours_v7.parquet           (Per-frame note contour sidecar for v7 notes. Schema is identical to the v3 / v4 / v5 / v6 sidecar; the F0 contour is v6 plus residual split and optional ridge rescue, and `subharmonic_octave` is reserved / unused and always written as 0 — ADR-074)
/exports/
  event_encoders/{job_id}/notes_{extractor_version}.mid    (Piano Roll Notes MIDI for the last-exported window — produced atomically with audio_{extractor_version}.flac. v3, v4, v5, v6, and v7 use MPE Lower Zone with per-voice pitch bend; legacy v1/v2 use the slim seven-channel layout from ADR-067. ADR-068, ADR-069, ADR-070, ADR-071, ADR-072, ADR-074)
