...
**Acceptance criteria:**
- [x] Sequence Models response schemas expose `start_timestamp` / `end_timestamp` for CEJ spans, HMM overlay points, and HMM exemplars.
- [x] HMM detail response includes `region_detection_job_id`, `region_start_timestamp`, and `region_end_timestamp`.
- [x] HMM artifact endpoints read canonical timestamp fields and reject missing canonical fields with clear errors.
- [x] Sequence Models docs no longer describe `start_time_sec` / `end_time_sec` as persisted or API fields.
...
**Acceptance criteria:**
- [x] Continuous embedding parquet schema replaces `start_time_sec` / `end_time_sec` with `start_timestamp` / `end_timestamp`.
- [x] Continuous embedding manifest spans use `start_timestamp` / `end_timestamp`.
- [x] Worker internals may use relative geometry, but every persisted row/span adds the source `RegionDetectionJob.start_timestamp`.
- [x] Existing idempotency behavior remains unchanged.
- [x] Tests use `RegionDetectionJob.start_timestamp != 0` and assert persisted timestamps are epoch values.
...
**Acceptance criteria:**
- [x] HMM states parquet copies `start_timestamp` / `end_timestamp` from the CEJ parquet.
- [x] PCA/UMAP overlay parquet emits `start_timestamp` / `end_timestamp`.
- [x] Exemplar JSON emits `start_timestamp` / `end_timestamp`.
- [x] Label distribution consumes epoch HMM state timestamps directly and no longer adds the region job start timestamp.
- [x] Tests guard against double-offset bugs by using nonzero region job start timestamps.
...
**Acceptance criteria:**
- [x] Region audio-slice endpoint accepts `start_timestamp` as the canonical query parameter.
- [x] Endpoint resolves audio using the epoch start directly and validates the requested range against the region job epoch bounds.
- [x] Frontend helper `regionAudioSliceUrl` takes `startTimestamp` rather than job-relative `startSec`.
- [x] Existing Region Detection Timeline passes epoch coordinates directly.
- [x] Any remaining backend-only `start_sec` compatibility is either removed or limited to a deliberate test-only transition point.
...
**Acceptance criteria:**
- [x] TypeScript interfaces use `start_timestamp` / `end_timestamp` for Sequence Models span/window records.
- [x] Continuous Embedding detail page labels and displays epoch timestamp fields accurately.
- [x] HMM detail page derives spans from epoch state rows and passes epoch `jobStart` / `jobEnd` into `TimelineProvider`.
- [x] HMM State Timeline Viewer requests spectrogram tiles for the correct tile indices on nonzero epoch jobs.
- [x] HMM audio playback uses epoch `start_timestamp` via the canonical region audio helper.
- [x] `SpanNavBar` and `HMMStateBar` prop names no longer use ambiguous `Sec` / `start_time_sec` naming.
...
**Acceptance criteria:**
- [x] Script is dry-run by default and rewrites only with `--apply`.
- [x] Script rewrites CEJ parquet, CEJ manifest, HMM states parquet, HMM overlay parquet, and HMM exemplar JSON from legacy fields to canonical fields.
- [x] Script adds `RegionDetectionJob.start_timestamp` when legacy values are job-relative.
- [x] Script leaves already-canonical artifacts unchanged.
- [x] Script aborts on ambiguous timestamp values instead of guessing.
- [x] Script writes parquet and JSON artifacts atomically.

**Tests needed:**
- Unit tests using temp storage fixtures for relative legacy artifacts, already-canonical artifacts, and ambiguous artifacts.
...
**Acceptance criteria:**
- [x] `rg "start_time_sec|end_time_sec" src/humpback frontend/src/api frontend/src/components/sequence-models docs/reference/sequence-models-api.md docs/specs/2026-04-27-sequence-models-design.md docs/specs/2026-04-28-hmm-state-timeline-viewer-design.md` returns no Sequence Models contract usages.
- [x] Any remaining matches are either tests for migration from legacy artifacts or internal non-contract helpers with explicit comments.
- [x] Sequence Models APIs fail clearly when artifacts still contain only legacy fields after migration.
...
6. `cd frontend && npx vitest run src/components/sequence-models/timelineTileIndex.test.ts`
7. `cd frontend && npx playwright test e2e/sequence-models/hmm-sequence.spec.ts`
8. `uv run python scripts/migrate_sequence_model_timestamps.py`
9. `rg "start_time_sec|end_time_sec" src/humpback frontend/src/api frontend/src/components/sequence-models docs/reference/sequence-models-api.md docs/specs/2026-04-27-sequence-models-design.md docs/specs/2026-04-28-hmm-state-timeline-viewer-design.md`
