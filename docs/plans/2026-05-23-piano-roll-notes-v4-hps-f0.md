...
- [ ] `_score_f0_candidates` implements the per-frame scoring exactly per spec §4.2: max-magnitude in ±`cents_tolerance` window per harmonic, `floor + k_noise·MAD` per-frame threshold, `contribution = max(0, m_n − floor)`, `count_present` gate against `low_band_min_harmonics` / `high_band_min_harmonics`, sub-`low_band_threshold_hz` penalty, tie-break toward the smallest divisor (highest candidate frequency — Occam's razor; the ridge wins over an artifact divisor that scores identically via incidental harmonic alignments).
