...
- [ ] Page component loads the job via `useEventEncoderJob`, shows loading and
  error states
- [ ] When the job is complete, fetches timeline data via
  `useEventEncoderTimeline` with the selected k
- [ ] Top toolbar renders: back link to detail page, job ID, event count,
  duration, token count, Y-mode selector, frequency max selector, unvoiced
  mode selector, play/pause button, k-value selector
- [ ] Status bar renders at bottom with placeholder cursor info and keyboard
  shortcut hints
- [ ] Full-viewport layout: toolbar + canvas area + status bar, no scrolling
...
**Acceptance criteria:**
- [ ] Canvas element fills the available space between toolbar and status bar,
  scaled by `devicePixelRatio` for Retina
- [ ] Frequency grid lines drawn with Hz labels on the Y axis
- [ ] Time grid lines drawn with mm:ss labels relative to recording start
- [ ] Each event rendered as a rectangle: X from start/end timestamps, Y
  center from median F0 (or peak frequency based on Y-mode), height from F0
  range, minimum 4 px height and 2 px width
- [ ] Rectangle fill color from `labelColor(token_id, k)` with opacity scaled
  by voicing fraction
- [ ] Unvoiced events (voicing ≤ 0.3) rendered according to the selected
  unvoiced mode: at peak frequency with solid border and light fill, in a
  bottom lane, or hidden
- [ ] Ridge slope line drawn inside rectangles at least 12 px wide and 6 px
...
- [ ] Token label with opaque background pill rendered centered inside
  rectangles at least 20 px wide and 10 px tall
- [ ] Events outside the visible viewport are frustum-culled (not drawn)
...
**Acceptance criteria:**
- [ ] Scroll (no modifier) zooms the time axis centered on cursor
- [ ] Shift+Scroll zooms the frequency axis centered on cursor
- [ ] Drag pans the time axis only (no frequency panning)
- [ ] Zoom has minimum and maximum bounds (time: 2 s to full recording,
  frequency: clamped 0–5000 Hz)
- [ ] F key fits all events in view
- [ ] +/= zooms in, - zooms out (time axis)
- [ ] ArrowLeft/ArrowRight pans by 10% of viewport span
- [ ] Escape clears selection and token filter
- [ ] All keyboard shortcuts suppressed when focus is in input/textarea/select
  or when meta/ctrl/alt modifiers are held
- [ ] Status bar updates with cursor time, cursor frequency, and current zoom
  span on mouse move
...
**Acceptance criteria:**
- [ ] Click on an event selects it (white border highlight), does not activate
  token filtering
- [ ] Hover shows a tooltip with: token label (colored), duration, median F0,
...
- [ ] Double-click zooms to fit the clicked event with padding
- [ ] Token legend panel (top-right, collapsible) lists all token families
  with color swatch, label, count, and mean F0 or "noise"
- [ ] Clicking a token in the legend dims all non-matching events; clicking
...
**Acceptance criteria:**
- [ ] A 240 × 40 px canvas in the bottom-right corner of the main canvas area
- [ ] Shows all events as 2 px marks at their time × frequency position,
  colored by token
- [ ] White rectangle outline shows the current viewport bounds
- [ ] Clicking the minimap centers the viewport on the clicked time position
- [ ] Minimap redraws when viewport or data changes
...
**Acceptance criteria:**
- [ ] Play/pause button in the toolbar and Space keyboard shortcut control an
  HTML audio element
- [ ] When an event is selected, playback uses `regionAudioSliceUrl` with the
  event's start timestamp and duration (end - start), bounded to a minimum of
  0.1 s
- [ ] When no event is selected, playback uses `regionAudioSliceUrl` with the
  viewport start and the viewport span capped at 30 s
- [ ] Play/pause state is reflected in the button icon (Play/Pause from
  lucide-react)
- [ ] Audio element stops and resets when a new event is selected or k changes
- [ ] Playback does not scroll the canvas
...
**Acceptance criteria:**
- [ ] A link or button on the Event Encoder detail page navigates to
  `/app/sequence-models/event-encoder/{jobId}/piano-roll`
- [ ] Link is only rendered when the job status is "complete"
- [ ] Piano roll back link navigates to
...
**Acceptance criteria:**
- [ ] Piano roll spec uses mocked timeline data with at least 5 events
  spanning two token families and both voiced/unvoiced events
- [ ] Asserts: route renders, toolbar shows job stats, canvas element present,
  k-value selector present when multiple k values, back link navigates to
  detail page, play button exists
- [ ] Existing event-encoder spec extended: detail page shows piano roll link
  for a complete job
- [ ] All tests pass alongside existing event encoder tests
