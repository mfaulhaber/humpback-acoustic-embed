...
**Acceptance criteria:**
- [x] A reusable dialog supports single-resource and multi-resource delete confirmation copy.
- [x] Dialog title follows `Delete [resource type]`.
- [x] Dialog body identifies the selected resource or count and explains the consequence.
- [x] Dialog footer uses `Cancel` and `Delete`; `Cancel` closes without side effects.
- [x] Delete trigger and confirm buttons can use one shared rounded red background with white text style.
- [x] The shared component has focused tests for rendering, cancel, and confirm behavior.
...
- Create: `frontend/src/components/call-parsing/EventDetailPanel.test.tsx`

**Acceptance criteria:**
- [x] Call Parsing bulk delete flows use the shared confirmation dialog instead of the classifier-specific bulk dialog.
- [x] Call Parsing single-row Delete buttons open the shared dialog before mutations run.
- [x] Model and training-job delete flows no longer use native browser `confirm()`.
- [x] Review Delete Event buttons open a confirmation dialog before marking a pending delete correction.
- [x] Existing error toasts, query invalidations, selection clearing, and disabled states are preserved.
- [x] All in-scope Delete buttons use the shared delete button style.
...
- Create: `frontend/src/components/call-parsing/TrainingDatasetTable.test.tsx`
- Modify: `tests/integration/test_call_parsing_router.py`
- Modify: `tests/unit/test_call_parsing_service.py`

**Acceptance criteria:**
- [x] `DELETE /call-parsing/segmentation-training-datasets/{dataset_id}` returns `204` after deleting an existing dataset.
- [x] The delete service removes the dataset row and associated sample rows.
- [x] The delete route returns `404` for a missing dataset.
- [x] The delete route returns `409` when a queued or running segmentation training job references the dataset.
- [x] Completed/failed training-job rows and produced models are not deleted as part of dataset deletion.
- [x] The frontend exposes a delete client function and mutation hook that invalidates the segmentation training dataset query.
- [x] Segment Training dataset rows include a Delete button using the shared style and confirmation dialog.
...
**Acceptance criteria:**
- [x] Continuous Embedding bulk delete uses the shared confirmation dialog.
- [x] Continuous Embedding row Delete opens the shared dialog before the mutation runs.
- [x] Event Encoder row Delete opens the shared dialog before the mutation runs.
- [x] Review/report links and active job Cancel behavior remain unchanged.
- [x] All in-scope Delete buttons use the shared delete button style.
...
- [x] No unrelated delete behavior outside Call Parsing and Sequence Models is changed.
- [x] No new native browser `confirm()` calls are introduced for delete actions.
- [x] Documentation remains limited to the design and implementation plan unless implementation reveals a domain-local rule that belongs in a capsule.
...
5. `cd frontend && npx vitest run src/components/shared/DeleteConfirmationDialog.test.tsx src/components/call-parsing/TrainingDatasetTable.test.tsx src/components/call-parsing/EventDetailPanel.test.tsx src/components/call-parsing/SegmentReviewWorkspace.test.tsx src/components/call-parsing/WindowClassifyReviewWorkspace.test.tsx src/components/sequence-models/ContinuousEmbeddingJobTable.test.tsx src/components/sequence-models/EventEncoderJobTable.test.tsx`
